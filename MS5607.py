...
class MS5607:
	...
	def __convert_pressure_temperature(self, pressure, temperature):
		...
		temp = 2000 + dT*(self._coefficients[5]/(2**23))
	...
	def get_altitude(self, samples,write_file=None):
		press_accum = 0
		temp_accum = 0
		for n in range(0, samples):
			...
			(press_conv, temp_conv) = self.__convert_pressure_temperature(pressure, temperature)
			press_accum += press_conv
			temp_accum += temp_conv
			if write_file:
				write_file.write('{},{},{}\n'.format(press_conv,temp_conv,time.time()))
		temp_avg = temp_accum / samples
		press_avg = press_accum / samples
		return (self.__pascal_to_cm(press_avg),temp_avg)
